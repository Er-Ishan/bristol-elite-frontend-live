...
export async function validatePromoCode(
    ...
    try {
        const res = await fetch(`${API}/api/promocode/${encodeURIComponent(code.trim())}`);
        const data = await res.json();
...
export async function fetchCancellationCharge(): Promise<CancellationCharge | null> {
    try {
        const res = await fetch(`${API}/api/cancellation/charges`);
        const data = await res.json();
...
export async function fetchBookingFees(): Promise<number> {
    try {
        const res = await fetch(`${API}/api/booking-fees`);
        const data = await res.json();
        if (data.success) {
...
export async function fetchTerminalsByProduct(productId: number) {
    const res = await fetch(`${API}/api/data/terminals-by-product/${productId}`);
    return res.json();
}

export async function fetchBookingByIdentifier(
    idOrRef: string
): Promise<{ success: boolean; data?: BookingReceiptData; message?: string }> {
    const res = await fetch(`${API}/api/booking/${encodeURIComponent(idOrRef)}`);
    const json = await res.json();

    if (!json.success || !json.data) {
        return { success: false, message: json.message || "Booking not found" };
    }
    ...
    return {
        ...
        data: {
            ...row,
            booking_id: row.id as number | string,
        } as BookingReceiptData,
    };
}
...
export async function notifyPaymentSessionExpired(
    ...
    try {
        const res = await fetch(`${API}/api/stripe/payment-session-expired`, {
            method: "POST",
            headers: {
...
export async function completeBookingAfterPayment(payload: {
    ...
    const res = await fetch(`${API}/api/create-booking-after-payment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
